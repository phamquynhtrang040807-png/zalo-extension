...
Service `zalo-bridge` dùng trực tiếp thư viện cục bộ `zalo-api-final`. Bridge lưu phiên đăng nhập trong Docker volume, tra số điện thoại thành UID rồi thực hiện kết bạn hoặc nhắn tin. Backend gọi bridge qua mạng Docker nội bộ:
...
Các biến template tin nhắn: `{username}`, `{followers}`, `{gmv}`. Có thể tạm dừng ngay từ trang Options của extension; worker giữ task trong hàng đợi và không tính lần retry khi đang tạm dừng. Bridge lưu kết quả theo `idempotency_key` để retry không gửi trùng.
...
`zalo-api-final` sử dụng giao thức tài khoản cá nhân không chính thức; thay đổi phía Zalo có thể làm phiên hết hạn hoặc API ngừng hoạt động, và tự động hóa có thể ảnh hưởng tài khoản. Giữ tốc độ gửi thấp và chỉ liên hệ người đã đồng ý nhận tin.
